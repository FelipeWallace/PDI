...
nova_image_horizontal = np.zeros([m, n])
nova_image_vertical = np.zeros([m, n])
...
for i in range(1, m-1):
    for j in range(1, n-1):
        ...
        nova_image_horizontal[i, j] = int(valor1)
...
for i in range(1, m-1):
    for j in range(1, n-1):
        ...
        nova_image_vertical[i, j] = int(valor1)

for i in range(0, m):
    for j in range(0, n):
        pontod = nova_image_horizontal[i, j]
        pontoe = nova_image_vertical[i, j]

        img_final[i, j] = np.sqrt(np.sum(np.square(pontod-pontoe)))
